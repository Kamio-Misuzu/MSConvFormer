...
try:
    ...
    # Fallback to built-in sorting if natsort is unavailable.
    natsorted = sorted
...
class ADFTDLoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        ...
        # For data-efficiency experiments, optionally subsample TRAIN subjects only.
        if flag == "TRAIN":
            ratio = float(getattr(args, "train_ratio", 1.0) or 1.0)
            subset_seed = int(getattr(args, "train_subset_seed", 42))
            self.train_ids = self._subsample_subject_ids(
                self.label_path, self.train_ids, ratio, subset_seed
            )
    ...
    def _subsample_subject_ids(self, label_path, subject_ids, ratio, seed):
        """
        Subsample TRAIN subject IDs in a class-balanced way.
        Only used for flag == "TRAIN" to keep VAL/TEST fixed.
        """
        ...
        data_list = np.load(label_path)  # col0: class label, col1: subject id
        id_to_label = {}
        ...
        for sid in subject_ids:
            sid_int = int(sid)
            lbl = id_to_label.get(sid_int, None)
            if lbl is None:
                continue
            grouped.setdefault(lbl, []).append(sid_int)
        ...
        for _, ids in grouped.items():
            ...
            chosen = rng.choice(ids, size=n_keep, replace=False).tolist()
            sampled_ids.extend(chosen)

        sampled_ids = sorted(sampled_ids)
        print(
            f"[ADFTDLoader] train_ratio={ratio:.2f}, "
            f"subjects {len(subject_ids)} -> {len(sampled_ids)} (seed={seed})"
        )
        return sampled_ids
